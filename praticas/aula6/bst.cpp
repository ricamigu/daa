// Exemplo de uso de Ãrvores BinÃ¡rias de Pesquisa Equilibradas (BSTs)
// -------------------------------------
...
using namespace std;  // Para nÃ£o ser necessÃ¡rio escrever std::set, std::map, etc
...
int main() {
  ...
  s.insert(6);
  s.insert(8);
  // O cast (int) Ã© para converter, visto que size() retorna size_type que
  // Ã© realmente um long unsigned int
  printf("Tamanho: %d\n", (int)s.size()); 
  
  printf("Procurando o numero 2: ");
  if (s.find(2)!=s.end()) printf("descobri o 2 :)\n");
  else printf("nao existe o 2... :(\n");
  ...
  // "auto" implica que a variÃ¡vel fica com o tipo determinado automaticamente
  ...
  if (m.find("Pedro")!=m.end()) printf("Encontrei o Pedro no mapa\n");
  else                          printf("Nao encontrei o Pedro no mapa\n");
  if (m.find("Aniceto")!=m.end()) printf("Encontrei o Aniceto no mapa\n");
  else                            printf("Nao encontrei o Aniceto no mapa\n");
